...
gulp.task('usemin', ['wiredep','clean','images','templates','copy:scripts','copy:root','sass:build','fonts'], function() {
  return gulp
    .src(config.src + '/index.html')
    .pipe(gulp.dest(config.dist))
    .pipe($.htmlReplace({
    ...
    .pipe(gulp.dest(config.dist))
    .pipe($.usemin({
      //main: [$.uglify()], <-- should work, but throws vague error.
      css: ['concat', $.cssnano({
        autoprefixer: {browsers: config.autoprefixer, add: true}
      })]
    }))
 ...
 */
gulp.task('build', ['usemin', 'htmlmin'], function() {
});
